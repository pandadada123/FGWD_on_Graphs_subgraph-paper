...
class Graph():
    ...
    def get_attr(self,vertex):
        ...
        # return self.nx_graph._node[vertex]
        return self.nx_graph._node[vertex].get('attr_name')
...
# def graph_colors(nx_graph,vmin=0,vmax=7):  # How to draw the color properly
#     cnorm = mcol.Normalize(vmin=vmin,vmax=vmax)
#     cpick = cm.ScalarMappable(norm=cnorm,cmap='viridis')
#     cpick.set_array([])
#     val_map = {}
#     V_old = {} # different the attribute names nodes
#     # V_new = {}
#     for k,v in nx.get_node_attributes(nx_graph,'attr_name').items():
#         V_old[k]=v       
#     lis = list(V_old.values()) 
#     temp_set = {tuple(row) for row in lis} # use set to get unique attributes of nodes
#     V_new = np.array(list(temp_set))
#     # lis =[] # create empty list
#     # tt= np.array([],[])
#     colors=[]
#     for node in V_old: # iterate keys for all nodes in graph # for node in nx_graph.nodes():
#         # print(node)
#         pos=np.where((V_new == V_old[node]).all(axis=1))
#         pos=pos[0][0]+1     # new color indexes of each node            
                
#         val_map[node]=cpick.to_rgba(pos)
...
def draw_rel(G,draw=True,shiftx=0,shifty=0,return_pos=False,with_labels=True,swipy=False,swipx=False,vmin=0,vmax=7):
    ...
    if with_labels:
        ...
        # nx.draw(G,pos,with_labels=True,labels=Keys_dict,node_color = colors)  
