...
# values = [1e-9,1.0]
# values = [1e-9]
values = [1e-9,1e-6,1e-3,1e-2,5e-2,
                  1e-1,5e-1,1.0]
# values = [1e-6,1e-3,1e-2,5e-2,
#                   1e-1,5e-1]
values = [1e-20]
# values = [1e-1,5e-1,1.0]
...
for epsilon in values:
    ...
    # Append the rates and other statistics to the result list
    result_list.append(["Rate 1: FGWD is almost zero", yes1/Effective_Num])
    result_list.append(["Rate 2: find the approx matching", yes2/Effective_Num])
    result_list.append(["Rate 3: the matching is exactly right", yes3/Effective_Num])
    result_list.append(["Rate 4: the ratio of correct nodes", np.nanmean(Ratio)])
    result_list.append(["Average time", np.nanmean(time_x)])
    result_list.append(["Num_not_connected", Num_not_connected])
    result_list.append(["Number of nan in DFGW", np.isnan(DFGW).sum()])
    result_list.append(["Mean of DFGW", np.nanmean(DFGW)])
    result_list.append(["Std of DFGW", np.nanstd(DFGW)])
    
    Result_list.append(result_list)
    Info_list.append([yes3/Effective_Num,np.nanmean(time_x)])
